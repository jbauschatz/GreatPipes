...
public class StickAndBeamDrawer {
	public void draw(Graphics g) {
		...
		int bottomLine = measure.getLineView().getYForPitch(Note.LOW_G) + stickLength;
		...
		if (notes.length == 1) {
			...
			for (int d = Note.EIGHTH; d<=v.getNote().denominator; d*=2) {
			...
			for (int n = 0; n<notes.length-1; ++n) {
				...
				for (int d = Note.EIGHTH; d<=v.getNote().denominator; d*=2) {
					if (d <= right.getNote().denominator) {
			...
			int beamY = bottomLine;
			for (int d = Note.EIGHTH; d<=right.getNote().denominator; d*=2) {
				if (d <= penult.getNote().denominator) {
